...
class AGVAgent implements TickListener, MovingRoadUser {
    ...
    void nextDestination() {
        ...
        path = new LinkedList<>(roadModel.get().getShortestPathTo(this,
                destination.get()));
        /*if(roadModel.get().hasRoadUserOn(roadModel.get().getPosition(this), path.peek())) {
            path = new LinkedList<>(roadModel.get().getPathTo(this,
                    destination.get()));
        }*/
    }
    ...
    public void tick(TimeLapse timeLapse) {
        try {
            ...
            Point b = path.poll();
            System.out.println("check " + path.peek());
            if (roadModel.get().hasRoadUserOn(b, path.peek())) { //roadModel.get().getPosition(this), destination.get())) {
                System.out.println("from  " + a + " to " +  path.peek());
                nextDestination();
            } else {
                System.out.println("check " + path.peek());
                roadModel.get().moveTo(this, b, timeLapse);
                path.remove();
